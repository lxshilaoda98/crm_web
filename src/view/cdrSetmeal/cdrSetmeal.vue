<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="套餐名称" prop="Name" width="120" />
        <el-table-column align="left" label="备注" prop="Remarks" width="120" />
        <el-table-column align="left" label="租用单位[月]" prop="RentalUnit" width="120" />
        <el-table-column align="left" label="租用租金" prop="RentalMoney" width="120" />
        <el-table-column align="left" label="免费分钟数" prop="RentalMinute" width="120" />
        <el-table-column align="left" label="按钮组">
            <template #default="scope">
            <el-button type="text" icon="el-icon-edit" size="small" class="table-button" @click="updateCdrSetmeal(scope.row)">变更</el-button>
...
export default {
  ...
  // 条件搜索前端看此方法
    ...
    async onDelete() {
      ...
      const res = await deleteCdrSetmealByIds({ ids })
      if (res.code === 0) {
        this.$message({
          type: 'success',
          message: '删除成功'
    ...
    async updateCdrSetmeal(row) {
      const res = await findCdrSetmeal({ ID: row.ID })
      this.type = 'update'
      if (res.code === 0) {
    ...
    async deleteCdrSetmeal(row) {
      const res = await deleteCdrSetmeal({ ID: row.ID })
      if (res.code === 0) {
        this.$message({
          type: 'success',
          message: '删除成功'
