<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="线路号码" prop="User" width="120" />
        <el-table-column align="left" label="密码" prop="PassWord" width="120" />
        <el-table-column align="left" label="消费" prop="Consumption" width="120" />
        <el-table-column align="left" label="所属账号" prop="Costomer" width="120" />
        <el-table-column align="left" label="归属" prop="Gs" width="120" />
        <el-table-column align="left" label="区号" prop="Qh" width="120" />
        <el-table-column align="left" label="按钮组">
            <template #default="scope">
            <el-button type="text" icon="el-icon-edit" size="small" class="table-button" @click="updateCdrLine(scope.row)">变更</el-button>
...
export default {
  ...
  // 条件搜索前端看此方法
    ...
    async onDelete() {
      ...
      const res = await deleteCdrLineByIds({ ids })
      if (res.code === 0) {
        this.$message({
          type: 'success',
          message: '删除成功'
    ...
    async updateCdrLine(row) {
      const res = await findCdrLine({ ID: row.ID })
      this.type = 'update'
      if (res.code === 0) {
    ...
    async deleteCdrLine(row) {
      const res = await deleteCdrLine({ ID: row.ID })
      if (res.code === 0) {
        this.$message({
          type: 'success',
          message: '删除成功'
