<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="费率名称" prop="Name" width="120" />
        <el-table-column align="left" label="备注" prop="Remarks" width="120" />
        <el-table-column align="left" label="创建人" prop="Create" width="120" />
        <el-table-column align="left" label="规则时间" prop="ChargingTime" width="120" />
        <el-table-column align="left" label="超规则收费" prop="LocalMoneyS" width="120" />
        <el-table-column align="left" label="本地市话" prop="LocalMoney" width="120" />
        <el-table-column align="left" label="国内长途" prop="DomesticMoney" width="120" />
        <el-table-column align="left" label="国际长途" prop="InternationMoney" width="120" />
        <el-table-column align="left" label="套餐" prop="SetMeal" width="120" />
        <el-table-column align="left" label="按钮组">
            <template #default="scope">
            <el-button type="text" icon="el-icon-edit" size="small" class="table-button" @click="updateCdrRate(scope.row)">变更</el-button>
...
export default {
  ...
  // 条件搜索前端看此方法
    ...
    async onDelete() {
      ...
      const res = await deleteCdrRateByIds({ ids })
      if (res.code === 0) {
        this.$message({
          type: 'success',
          message: '删除成功'
    ...
    async updateCdrRate(row) {
      const res = await findCdrRate({ ID: row.ID })
      this.type = 'update'
      if (res.code === 0) {
    ...
    async deleteCdrRate(row) {
      const res = await deleteCdrRate({ ID: row.ID })
      if (res.code === 0) {
        this.$message({
          type: 'success',
          message: '删除成功'
